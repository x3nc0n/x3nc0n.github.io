...
And then it shifts again. As I built repo after repo, I realized there's an even better baseline than the one I'd been standardizing on. Instead of federating GitHub's OIDC token to an *app-registration service principal*, you can federate it directly to a **User-Assigned Managed Identity (UAMI)** — one per repo, or even per Action. The workflow side looks identical (`azure/login@v2`, `id-token: write`), but the Azure-side identity becomes a managed identity you provision and RBAC-scope *as code*: no app registration to create, no client secret that ever has to exist, no Entra app-registration sprawl to govern, and no separate admin-consent dance. It's the pattern I wish I'd started with — and across this series you'll see OIDC-to-service-principal used throughout, because that's what the repos actually shipped with. I'm now migrating all of them to OIDC-to-UAMI, and a dedicated post later in this series walks through that overhaul. The headline: with the agent workflow driving it, retrofitting the auth pattern across *every* repo's Actions is a job measured in **hours, not the weeks-to-never** it would have been by hand. Watch for it.
